							    ...
							    <?php
							        if($_SERVER['REQUEST_METHOD']=="POST"){
                                        ...
                                        try{
											...
											$old_db=mysqli_connect($old_db_host,$old_db_username,$old_db_password,$old_db_name);
											if(empty($old_db)){
											    throw new Exception('cant connect to old database!');
											}
                                            mysqli_set_charset($old_db,'utf8');
											//connection to new db
											$pdo=mysqli_connect($db_host,$db_username,$db_password,$db_name);
											if(empty($pdo)){
											    throw new Exception('cant connect to new database!');
											}
                                            ...
                                            $users=fetchAll($old_db,mysqli_query($old_db,$query));
                                            if(!empty($users)){
                                                foreach($users as $user){
													...
													$exists=fetchAll($pdo,mysqli_query($pdo,$query));
                                                    if(!empty($exists)){
											...
											$departments=fetchAll($old_db,mysqli_query($old_db,$query));
											if(!empty($departments)){
                                                foreach($departments as $department){
                                                    ...
                                                    if(!empty($users)){
														...
														if($user_index!==false){
															...
															if(!empty($user_email)){
																...
																$staff=fetchAll($pdo,mysqli_query($pdo,$query));
																if(!empty($staff)){
																	$user_id=$staff[0]['id'];
																}
															}
													...
													mysqli_query($pdo,$query);
													$department_id=mysqli_insert_id($pdo);
													//import staffs
													if(!empty($user_id)){
											...
											$user=fetchAll($pdo,mysqli_query($pdo,$query));
											if(!empty($user)){

                                                $user_id=$user[0]['id'];
                                                $query="select * from posts";
                                                $news=fetchAll($old_db,mysqli_query($old_db,$query));
                                                foreach($news as $new){
                                                    ...
                                                    if(!empty($departments)){
														...
														if($department_index!==false){
															...
															if(!empty($department_name)){
																...
																$department=fetchAll($pdo,mysqli_query($pdo,$query));
																if(!empty($department)){
											...
											$tickets=fetchAll($old_db,mysqli_query($old_db,$query));
