...
trait Download {
    ...
    public function prepare($file, $delay = 0) {
        if (! is_file($file)) {
    ...
    public function processDownload($type='attachment',$name=null) {
        $ranges = NULL;
        $t = 0;
        ...
        header(sprintf('Content-Disposition: '.$type.'; filename="%s"', $this->name));
