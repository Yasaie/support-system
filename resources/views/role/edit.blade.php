...
@section('title','- نوع کاربری')
@section('description','نوع کاربری')
@section('keywords','نوع کاربری')
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box clearfix">
                                <h5 class="card-header m-b-20 b-0">ویرایش نوع کاربری</h5>
                                @include('partial.flashMessage')
                                <form action="{{ route('role.update',$role) }}" method="POST" role="form" class="form-horizontal">
                                    {{ csrf_field() }}
                                    {{ method_field('PATCH') }}
                                    <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-xs-12 col-sm-12">
                                            <div class="form-group {{ $errors->has('permissions') ? ' has-error' : '' }}">
                                                <select name="permissions[]" id="permissions" class="form-control select2" data-placeholder="انتخاب سطح دسترسی" multiple>
                                                    @if($role->permissions->isNotEmpty())
                                                        @foreach($role->permissions as $permission)
                                                            <option data-name="{{ $permission->name }}" selected value="{{ $permission->id }}">{{ $permission->name }}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                            </div>
