...
@section('title',' - ویرایش اعلامیه')
@section('description','ویرایش اعلامیه')
@section('keywords','ویرایش اعلامیه')
            ...
            <div class="content">
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <h5 class="card-header m-b-20 b-0">ایجاد ویرایش اعلامیه</h5>
                                @include('partial.flashMessage')
                                <form method="POST" class="form-horizontal" action="{{ route('notification.update',$notification) }}" role="form">
                                    {{ csrf_field() }}
                                    {{ method_field('PATCH') }}
                                    <div class="row">
                                        <div class="col-xs-12 col-sm-12">
                                            <div class="form-group {{ $errors->has('subject') ? ' has-error' : '' }}">
                                                ...
                                                <input name="subject" type="text" id="subject" value="{{ $notification->subject }}" class="form-control" placeholder="مثال: ارائه راهکار" required>
                                            </div>
                                        ...
                                        <div class="col-xs-12 col-sm-12">
                                            <div class="form-group {{ $errors->has('recipients') ? ' has-error' : '' }}">
                                                <label for="recipients">گیرنده پیام</label>
                                                <select name="recipients[]" id="recipients" class="select2 select2-multiple" multiple data-placeholder="گیرنده">
                                                    @if($notification->recipients->isNotEmpty())
                                                        @foreach($notification->recipients as $recipient)
                                                            <option selected data-name="{{ $recipient->name }}" value="{{ $recipient->id }}">{{ $recipient->name }}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                            </div>
                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <div class="form-group {{ $errors->has('message') ? ' has-error' : '' }}">
                                                <label for="message">محتوا</label>
                                                <textarea name="message" class="use-rich-editor" id="message" maxlength="250" rows="6" data-placeholder="محتوای خبر را وارد کنید...">{{ $notification->message }}</textarea>
                                            </div>
