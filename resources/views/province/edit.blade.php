...
@section('description','ویرایش استان')
@section('keywords','ویرایش استان')
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box clearfix">
                                <h5 class="card-header m-b-20 b-0">ویرایش استان</h5>
                                @include('partial.flashMessage')
                                <form action="{{ route('province.update',$province) }}" method="POST" role="form" class="form-horizontal">
                                    {{ csrf_field() }}
                                    {{ method_field('PATCH') }}
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6 col-sm-12 col-xs-12">
                                            <div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
                                                ...
                                                <input id="name" type="text" class="form-control" name="name" value="{{ $province->name }}" autofocus>
                                            </div>
                                            @if ($errors->has('name'))
                                                ...
                                                <select id="country" name="country" class="form-control">
                                                    @unless($countries->isEmpty())
                                                        @foreach($countries as $country)
                                                            <option value="{{ $country->id }}" {{ ($province->country_id==$country->id) ?' selected ':'' }}>{{ $country->short_name }} - {{ $country->name }}</option>
                                                        @endforeach
                                                    @endunless
                                                </select>
                                            </div>
